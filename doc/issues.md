...
1. 文档/代码收口项
- 管理看板已接入项目积分消耗统计与轻量 API 错误率。
- 素材查重与引用历史已从空实现改为真实候选评分与稿件引用扫描。
- 世界模块生成失败时记录 `FAILED` 和错误信息，不再写入占位内容。
- 设置页已暴露提示词 reset/metadata，并补齐帮助页接口驱动渲染。

2. 本轮验证
- 后端：`mvn -q test` 通过。
- 前端：`npm run test`、`npm run build` 通过。
- 详细记录见：`doc/test/2026-06-06-doc-code-closure.md`。
