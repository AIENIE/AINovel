...
- 接口依据：`/home/duwei/aienie-doc/interfaces/ai-service/` 的 `supports_streaming`、`ChatCompletionsStream` 和严格事件序列；AINovel 消费 proto 与相关字段编号一致。
- 删除审计：前端所有已有 DELETE 封装统一使用空响应请求；V7 将故事 → 角色/大纲 → 稿件及质量记录改为数据库级联，并保留其它 V2 表已有的 CASCADE/SET NULL 策略。
- 真实删除：`G2 验收临时故事`（含下游内容）、`L4 删除验收临时故事`（新建空故事）、`L4 AI 进度验收临时故事`（AI 创建）均返回 204；三个临时故事均已删除。
- AI 进度：任务 `4a1e5807-c03e-4324-87db-8009229e3fdb` 通过真实流式事件展示 token；首次运行暴露后台线程安全上下文缺口，修复后复用持久化任务重试并以 `attempt=2` 成功完成。
- 恢复行为：把已完成任务作为刷新恢复任务重新加载后，前端读取结果并跳转到对应工作台，同时清除 `sessionStorage` 跟踪键；浏览器控制台无错误。
- 截图证据：`/home/duwei/tmp/ainovel-l4-ai-progress.png`（不进入仓库）。
