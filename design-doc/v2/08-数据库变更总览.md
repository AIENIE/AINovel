...
| 25 | `plot_quality_runs` | 质量诊断 | 实装扩展 | 场景级剧情风险诊断运行记录 |
| 26 | `plot_quality_issues` | 质量诊断 | 实装扩展 | 剧情诊断问题证据与最小修复建议 |
...
说明：
- `source_text_hash` 用于生成候选和采纳候选时校验场景正文是否已变化。
- `rewrite_plan_json` 与 `surgical_fixes_json` 存储给作者审核的结构化修订建议。
- 趋势统计不落单独表，由 `plot_quality_runs` 每个场景最新记录实时聚合。

### 2.5 版本控制系统（→ 见 `04-版本控制系统.md` § 3.1）
...
### 2.8 工作台与体验优化（→ 见 `07-工作台与体验优化.md` § 3.1）
