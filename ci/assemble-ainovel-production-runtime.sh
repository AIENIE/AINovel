...
python3 "$repo/ci/write-production-migration-artifacts.py" "$out" "$out/release/production-migration-artifacts.json"; chmod 0444 "$out/release/production-migration-artifacts.json"
python3 "$repo/ci/verify-production-runtime-contract.py" "$out/release/production-runtime-contract.json" "$out/docker-compose.yml" ai-novel "$out/backend/start-production-backend.sh" "$out/backend/production-migration-entrypoint.sh" "$out/release/production-migration-executor"
python3 - "$out" <<'PY'
