  ...
  return (
    <div className="space-y-6 border-t border-zinc-200 pt-6" data-testid="outline-direction-workspace">
      <div className="grid gap-5 md:grid-cols-2">
        <div>
          <p className="text-xs font-medium text-zinc-500">{t("guided.currentRootDirection")}</p>
        ...
        <div className="border-l-0 border-zinc-200 md:border-l md:pl-5">
          <p className="text-xs font-medium text-zinc-500">
            {t("guided.latestDevelopmentDraft", { revision: Number(candidate.developmentRevision || 0) })}
          </p>
          {development ? (
            ...
            <p className="mt-2 text-sm leading-6 text-zinc-500">{t("guided.notDevelopedHint")}</p>
          )}
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          placeholder={t("guided.instructionPlaceholder")}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
        ...
        >
          <RefreshCcw className="mr-2 h-4 w-4" /> {t("guided.rewriteByFeedback")}
        </Button>
        <Button className="bg-zinc-950 text-white hover:bg-zinc-800" disabled={busy} onClick={() => void onExpand()}>
          {t("guided.selectAndExpand")} <ArrowRight className="ml-2 h-4 w-4" />
        </Button>
      </div>
    </div>
  );
