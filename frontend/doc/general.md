...
- **素材库（混合检索）**：手动创建、TXT/Markdown/PDF/DOC/DOCX 上传解析（Tika+Qdrant）、自动切片与嵌入；待审工作台；查重合并；引用历史；编辑/删除；检索与写作自动提示。
...
- 素材库 V2：手工创建、TXT/Markdown/PDF/DOC/DOCX 上传解析与状态轮询、混合检索、自动提示、待审工作台（批准/驳回）、去重与合并、引用历史、权限控制。
